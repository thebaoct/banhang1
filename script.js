...
function syncDebtStatusToHistory() {
    ...
    h.forEach(order => {
        if(order.payment === 'DEBT' && !order.isPaid) {
...
function deleteCurrentCustomer() {
    ...
    if (typeof currentUser !== 'undefined' && currentUser) {
        if(typeof deleteFromCloudCollection === 'function') deleteFromCloudCollection('Customers', phone);


        invoicesToDelete.forEach(inv => {
            if(typeof deleteFromCloudCollection === 'function') deleteFromCloudCollection('Invoices', inv.id);
        });


        debtsToDelete.forEach(d => {
            if(typeof deleteFromCloudCollection === 'function') deleteFromCloudCollection('Debts', d.id);
        });
    }
