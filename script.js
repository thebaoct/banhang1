...
function addNewCustomer() {
    ...
    if (!n && !p) return alert("Vui lòng nhập ít nhất Tên hoặc Số điện thoại!");
    if (p && customerList.some(c => c.phone === p)) return alert("Số điện thoại này đã tồn tại!");


    if (!n) n = "Khách " + p;
    if (!p) {
        let uniqueCode = "KH_" + Date.now().toString().slice(-6); 
        p = uniqueCode;
        alert("⚠️ Vì không có SĐT, hệ thống đã tự tạo mã định danh: " + uniqueCode);
    }


    customerList.push({name: n, phone: p});
    localStorage.setItem('thebao_customers', JSON.stringify(customerList));
